...
export default {
  name: 'Mute',
  usage: 'mjute <mention | user ID> [reason]',
  ...
  exec: async (ctx) => {
    ...
    ctx.worker.moderation.mute(ctx.guild.id, userID as any, time)
      .then(async (member) => {
        ...
        if (logChannel) {
          const msg = await ctx.worker.api.messages.send(logChannel.id, {
            embed: {
              title: `Muted Member (case #${moderationNum})`,
        ...
        ctx.worker.responses.tiny(ctx, ctx.worker.colors.RED, `Muted ${member.user.username}#${member.user.discriminator}`);
      })
      .catch(err => {
