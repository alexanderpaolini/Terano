...
export default {
  ...
  exec: async (ctx) => {
    ...
    if (!command) {
      await ctx.normalResponse(ctx.worker.colors.RED, 'No command was given, please include a command.')
      return
    }
    ...
    await ctx.normalResponse(ctx.worker.colors.ORANGE, `${cmd.disabled ? 'Disabled' : 'Enabled'} command **${cmd.name as string}**`)
  }
} as CommandOptions
