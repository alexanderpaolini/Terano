...
router.post('/vote', async (req, res) => {
  if (!req.vote || !req.vote.user) return res.status(400).json({ 'success': false });
  ...
  req.app.comms.sendWebhook(config.webhooks.votes.id as `${bigint}`, config.webhooks.votes.token, {
    embeds: [{
      title: `User Voted`,
      author: {
